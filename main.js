...
var numBeggers = 3;
var offerings = [1,1,5,9,6];

var beggerArray = [];
var beggerObject = {begger:[]};
for(var i = 0; i < numBeggers; i++){
  beggerArray.push(beggerObject);
  beggerArray[i].begger = [];
};

for(var n =0; n < offerings.length+1; n++){
  for(var j = 0; j < beggerArray.length; j++){
    if(offerings === undefined){
      break;
    }else{
      myOffering = offerings.shift();
      beggerArray[j].begger.push(myOffering);
      console.log(beggerArray[j].begger);
    };
  };
};
